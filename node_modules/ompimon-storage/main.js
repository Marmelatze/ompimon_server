...
expireTime = 500; //5 min


/**
 * Storage class for handling communication with the redis db
 *
 * ## Publish / Subscribe ##
 ...
 *
 * Each message is an array with one or more items. Each item has a `type` key, which can be `counter` or `send` to differentiate data from counter and send functions.
 *
 * ##### counter #####
     [
         {
...
_.extend(Storage.prototype, {
     ...
     * @async
     */
    killConnection: function(callback){
        rclient.quit();
        publishClient.quit();
    ...
    deleteApp: function(appId, callback){
        this._deleteKeys("app:"+appId, callback);
    },

    /**
     * sets a app to status finalize
     *
     * @method finalize
     * @param appId {Integer}
     * @param callback {Function}
     * @async
     */
    finalize: function(appId, callback){
        rclient.HMSET("app:"+appId,{
            "finalize": true,
     ...
     * @async
     */
    isFinalize: function(appId, callback){
        rclient.HGET("app:"+appId, "finalize", function(err, finalize){
     ...
     * @async
     */
    initAction: function(appId, data, callback){
...
*/
    /**
     * @method receiveTotalAction
     * @param appId {Integer}
     * @param data {Object}
     * @param callback {Function}
     * @async
     */
    receiveTotalAction: function(appId, data, callback){
        var self = this;
        var messageContainer = [];
        var tasks = [];
        this._getSendFunctionById(appId, data.sendId, function(funcName){
            data.ranks.forEach(function(rankData){
                _.each(rankData.data, function(sendData){
                    tasks.push(function(callback){
                        self._setSendTotalData(appId, rankData.rank, funcName, sendData, function(message){
                            messageContainer.push(message);
                            callback();
                        });
                    });
...
/*
{
    ...
    */
    ...
    /**
     * @method receiveSingleDetailsAction
     * @param appId {Integer}
     * @param data {Object}
     * @param callback {Function}
     * @async
...
*/
    /**
     * @method receiveTotalCounterAction
     * @param appId {Integer}
     * @param data {Object}
     * @param callback {Function}
     * @async
     */
    receiveTotalCounterAction: function(appId, data, callback){
        var self = this;
        var tasks = [];
        var messageContainer = [];
        data.forEach(function(rankData){
            _.each(rankData.counters, function(countVal, funcName) {
                tasks.push(function(callback) {
                    self._setCounterTotalData(appId, rankData.rank, funcName, countVal, function(message){
                        messageContainer.push(message);
                        callback();
                    });
                });
    ...
    getApplications: function(callback){
        var self = this;
        rclient.LRANGE("apps",0, -1, function(err, list){
            var ret = [];
            var tasks = [];
            _.each(list, function(name, appId){
                tasks.push(function(callback) {
                    self.isFinalize(appId, function(err, finalize){
                        if(!finalize){
                            ret.push({
                                name: name,
                                id: appId
     ...
     * @async
     */
    _setCounterTotalData: function(appId, rankId, funcName, countValue, callback){
        var self = this;
        var prefix = "app:"+appId+":rank:"+rankId+":counter:"+funcName+":total";
     ...
     * @async
     */
    _setSendTotalData: function(appId, rankId, funcName, data, callback){
        var self = this;
        var prefix = "app:"+appId+":rank:"+rankId+":to";

        rclient.SADD(prefix, data.rank,function(err){
            rclient.INCR(prefix+":"+data.rank+":"+funcName+":total", function(err, i){
                var message = {
                    "type": "totalsend",
                    "funcName": funcName,
                    "rank": rankId,
                ...
                rclient.HMSET(prefix+":"+data.rank+":"+funcName+":total:value:"+i, message);
                rclient.EXPIRE(prefix+":"+data.rank+":"+funcName+":total:value:"+i, expireTime);
                callback(message);
            });
        });
    },
