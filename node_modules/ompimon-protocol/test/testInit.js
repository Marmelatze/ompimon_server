...
module.exports = {
    ...
    testProcess: function (test) {
        var data2 = action.parse(client, stub.buildInit(stub.initData).get());

        action.process(client, data2, function (result) {
            test.equal(result.status, 0);
            test.deepEqual(client.ranks, data2.ranks);
            test.deepEqual(client.counterFunctions, data2.counters);
            test.deepEqual(client.sendFunctions, data2.sends);
    ...
    testAll: function (test) {
        ...
        Protocol.parse(client, buffer.get(), function (result) {
            ...
            test.deepEqual(client.counterFunctions, stub.initData.counters);
            test.done();
        });
    }
};
