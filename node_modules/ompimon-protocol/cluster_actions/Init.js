...
_.extend(Init.prototype, {
    ...
    process: function(client, data, callback) {
        monitor.message("cluster:"+client.id, "me", "init", data);
        var self = this;
        this._authenticate(client, data.username, data.password, function(result) {
            if (result.status != response.OK) {
                monitor.message("me", "cluster:"+client.id, "init_failed");
                callback(result);
            ...
            Storage.initAction(data, function(err, appId) {
                ...
                client.app.addClient(client, data.nodeId, client.ranks);

                var clientRedis = Storage.createConnection();

                if (0 !== data.nodeId) {
                    process.nextTick(function() {
                        monitor.message("me", "cluster:"+client.id, "init_ok");
    ...
    processClientMessage: function(client, channel, message) {
        ...
        switch (action) {
            ...
            case 'restart':
                ...
                break;
            case 'dataDetail':
                this.dataDetail(client, client.app.id, message);
                break;
        }
    },

    dataDetail: function (client, appId, message) {
        client.log("Send data detail for " + appId);
        this.apps[appId].handleDataRequest(message);
    }
});